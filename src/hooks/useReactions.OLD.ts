...
function useReactions(mediaIdRaw: string, userIdRaw?: string|null, refreshTrigger?: number) {
  ...
  // 🔥 Générer un userId guest si aucun userId n'est fourni
  const userId = useMemo(() => {
    if (userIdRaw) return String(userIdRaw)

    // Utiliser un guest ID stocké en localStorage pour persistance
  ...
  useEffect(() => {
    ...
    // Ne plus annuler les fetches précédents - laisser tous aboutir
    ...
    fetch(`/api/reactions?${qs}`, { cache: 'no-store', signal: ctrl.signal })
      ...
      .catch(err => {
        if (err?.name === 'AbortError') {
          console.log(`[useReactions] Fetch aborted for ${mediaId}`)
          return
        }
    ...
    return () => {
      // Ne pas annuler le fetch dans le cleanup - laisser les données arriver même si le composant se démonte
      // inflightGet.current?.abort()
    }
