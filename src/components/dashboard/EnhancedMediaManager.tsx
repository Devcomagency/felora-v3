...
import {
  Upload, Play, Image as ImageIcon, Diamond, Eye, EyeOff,
  Archive, MoreVertical, Check, ArrowLeft, Trash2,
  Grid, List, Search, Plus, TrendingUp, X, Zap,
  Smartphone, Monitor, Globe, Lock
} from 'lucide-react'
...
export default function EnhancedMediaManager() {
  ...
  const handleFileSelect = (files: FileList) => {
    ...
    if (file.type.startsWith('image/') || file.type.startsWith('video/')) {
      // Nettoyer l'ancienne URL de preview si elle existe
      if (filePreviewUrl) {
        URL.revokeObjectURL(filePreviewUrl)
      }

      // Créer la nouvelle URL de preview
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {/* Header Mobile-First avec design moderne */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          { label: 'Public', value: stats.public, color: 'emerald', icon: Globe },
      ...
      {uploadStep === 'select' && (
        <div className="bg-gray-800/40 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 md:p-6">
          <div
            className={`border-2 border-dashed rounded-xl p-6 md:p-8 text-center transition-all duration-300 cursor-pointer group relative ${
              dragActive 
            ...
            {dragActive && (
              <div className="absolute inset-0 bg-purple-500/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-purple-300 font-medium">Déposez votre fichier ici</p>
                </div>
              </div>
            )}
