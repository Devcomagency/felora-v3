...
export default function CameraScreen({ onClose, onCapture }: CameraScreenProps) {
  ...
  // Gestion appui long = vidéo, appui court = photo
  const handleMouseDown = useCallback(() => {
    longPressTimeoutRef.current = setTimeout(() => {
      startRecording()
    }, 500) // 500ms pour déclencher l'enregistrement
  }, [startRecording])
  ...
  }, [recordState, capturePhoto, stopRecording])
  ...
  const handlePublish = async () => {
    if (capturedVideo || capturedImage) {
      // Appeler onCapture pour traiter le média
      if (capturedVideo) {
        onCapture(capturedVideo)
      }
      
      // Fermer l'écran caméra
      onClose()
      
      // Rediriger vers le profil selon le rôle de l'utilisateur
      const userRole = (session?.user as any)?.role
      const userId = session?.user?.id
      
      if (userRole === 'ESCORT' && userId) {
        ...
        try {
          ...
          if (data.success && data.club?.handle) {
            router.push(`/profile-test/club/${data.club.handle}`)
          } else {
  ...
  if (cameraPermission === 'denied') {
    return (
      <div className="fixed inset-0 z-50 bg-[#0B0B0B] flex items-center justify-center p-4">
        <div className="text-center">
          <div className="w-20 h-20 mx-auto mb-6 rounded-full bg-red-500/20 flex items-center justify-center">
            <X className="w-10 h-10 text-red-500" />
          </div>
          <h2 className="text-xl font-bold mb-4">Permission refusée</h2>
          <p className="text-white/60 mb-6">
            L'accès à la caméra est nécessaire pour capturer du contenu.
          </p>
          <button
            onClick={onClose}
  ...
  // Écran caméra principal - Simplifié
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: '#0B0B0B', fontFamily: 'Inter' }}>
      {/* Bouton retour en haut à gauche */}
      <div className="absolute top-6 left-6 z-10">
      ...
      <div className="absolute top-6 right-6 z-10 flex items-center space-x-3">
        {/* Upload photo au-dessus du retourner caméra */}
        <div 
      ...
      {/* Preview caméra 9:16 avec arrondis - Même taille */}
      <div className="flex-1 relative p-4">
        <div className="relative w-full h-full max-w-sm mx-auto rounded-2xl overflow-hidden shadow-2xl" style={{ aspectRatio: '9/16' }}>
          <video
            ref={videoRef}
