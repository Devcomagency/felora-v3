...
export default function UploadDrop({ label, accept, maxMb = 20, onUploaded, onUploadedMeta }:{ label:string; accept:string; maxMb?:number; onUploaded:(url:string)=>void; onUploadedMeta?: (meta:{ url:string; key?: string })=>void }){
  ...
  const handle = useCallback(async (file: File) => {
    ...
    } catch (e:any) { setError(e?.message || 'Erreur upload') } finally { setBusy(false) }
  ...
  return (
    <div className="space-y-2">
      ...
      <label htmlFor={inputId} onDragOver={e=>{e.preventDefault(); e.stopPropagation()}} onDrop={onDrop} className="block p-4 rounded-xl border border-dashed border-white/20 bg-white/5 text-white/80 cursor-pointer hover:bg-white/10 transition-colors">
        {busy ? 'Upload…' : 'Glissez-déposez ou cliquez pour sélectionner'}
      </label>
