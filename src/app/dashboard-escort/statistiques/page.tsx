  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {(() => {
          const TopMediaList = dynamic(() => import('@/components/stats/TopMediaList'), { ssr: false, loading: () => <div className="h-24 bg-white/5 rounded-xl animate-pulse"/> })
          const onExport = () => exportCSV((topMediaQ.data||[]).map(i=>({rank:i.rank,type:i.type,title:i.title,likes:i.likes,views:i.views,score:i.score})), 'top-medias.csv')
          const onBoost = (id:string) => notifySuccess('Mise en avant', `Boost appliqué (mock) #${id}`)
          return (
            <TopMediaList items={topMediaQ.data||[]} loading={topMediaQ.isLoading} onExport={onExport} onBoost={onBoost} />
          )
        })()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {(() => {
          const ViewsBar = dynamic(() => import('@/components/stats/ViewsBar'), { ssr: false, loading: () => <div className="bg-white/5 rounded-2xl h-56"/> })
          return <ViewsBar series={viewsQ.data?.series||[]} loading={viewsQ.isLoading} />
        })()}
        {(() => {
          const RevenueDonut = dynamic(() => import('@/components/stats/RevenueDonut'), { ssr: false, loading: () => <div className="bg-white/5 rounded-2xl h-56"/> })
          return revenueBreakdownQ.data ? (
            <div className="bg-gray-800/40 backdrop-blur border border-gray-700/50 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold">Sources de revenus</h3>
              </div>
              <RevenueDonut orders={revenueBreakdownQ.data.orders} paywalls={revenueBreakdownQ.data.paywalls} gifts={revenueBreakdownQ.data.gifts} total={revenueBreakdownQ.data.total} />
            </div>
          ) : (
            <div className="bg-white/5 rounded-2xl h-56 animate-pulse"/>
          )
        })()}
      </div>
