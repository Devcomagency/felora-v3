...
import { AboutSection, ClubLocationSection, AvailabilitySection } from '../../../../../packages/ui/profile-test/Sections'
...
export default function ClubProfileTestPage() {
  ...
  return (
    <ErrorBoundary fallback={<ErrorFallback />}>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <section className="px-4 mt-[36px] md:mt-[48px]">
            ...
            <ProfileHeader
              ...
              online={true}
              languages={[]}
              services={profile.contact?.website ? [profile.contact.website.replace(/^https?:\/\//, '')] : []}
              stats={{
              ...
              showAvatar={false}
            />
            {/* Actions Bar personnalisée avec 3 boutons */}
            <div className="mt-4">
              <div className="flex gap-2">
                <button
                  onClick={() => setShowDetails(true)}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-purple-500/20 to-pink-500/20 border border-purple-500/30 text-white rounded-lg hover:from-purple-500/30 hover:to-pink-500/30 transition-all duration-200 text-sm font-medium"
                >
                  En savoir plus
                </button>
                <button
                  onClick={() => setShowAgenda(true)}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 border border-blue-500/30 text-white rounded-lg hover:from-blue-500/30 hover:to-cyan-500/30 transition-all duration-200 text-sm font-medium"
                >
                  Agenda
                </button>
                <button
                  onClick={() => setShowContact(true)}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-pink-500/20 to-rose-500/20 border border-pink-500/30 text-white rounded-lg hover:from-pink-500/30 hover:to-rose-500/30 transition-all duration-200 text-sm font-medium"
                >
                  Contact
                </button>
              </div>
      ...
      {/* Modal En savoir plus - Services/Équipements & Langues */}
      {showDetails && profile && (
        <div onClick={(e) => { if (e.target === e.currentTarget) setShowDetails(false) }} className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm flex items-end md:items-center justify-center p-4">
          <div className="glass-card w-full max-w-lg rounded-2xl p-5 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold text-lg">En savoir plus</h3>
              <button onClick={() => setShowDetails(false)} className="w-8 h-8 rounded-lg bg-white/10 text-white hover:bg-white/20 transition-colors">✕</button>
            </div>

            {/* Langues parlées */}
            {profile.languages && profile.languages.length > 0 && (
              <div className="mb-6">
                <h4 className="text-white font-medium mb-3 text-base">🌍 Langues parlées</h4>
                <div className="flex flex-wrap gap-2">
                  {profile.languages.map((language, index) => (
                    <span key={index} className="px-3 py-1.5 rounded-full text-sm bg-blue-500/20 border border-blue-500/30 text-blue-200">
                      {language}
                    </span>
                  ))}
                </div>
            ...
            {profile.services && profile.services.length > 0 && (
              <div className="mb-6">
                <h4 className="text-white font-medium mb-3 text-base">🏢 Services</h4>
                <div className="flex flex-wrap gap-2">
                  {profile.services.map((service, index) => (
                    <span key={index} className="px-3 py-1.5 rounded-full text-sm bg-purple-500/20 border border-purple-500/30 text-purple-200">
                      {service}
                    </span>
                  ))}
                </div>
            ...
            {profile.amenities && profile.amenities.length > 0 && (
              <div className="mb-4">
                <h4 className="text-white font-medium mb-3 text-base">🛠️ Équipements</h4>
                <div className="flex flex-wrap gap-2">
                  {profile.amenities.map((amenity, index) => (
                    <span key={index} className="px-3 py-1.5 rounded-full text-sm bg-green-500/20 border border-green-500/30 text-green-200">
                      {amenity}
                    </span>
                  ))}
                </div>
      ...
      {/* Modal Agenda */}
      {showAgenda && profile && (
        <div onClick={(e) => { if (e.target === e.currentTarget) setShowAgenda(false) }} className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm flex items-end md:items-center justify-center p-4">
          <div className="glass-card w-full max-w-md rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-semibold text-lg">📅 Agenda & Horaires</h3>
              <button onClick={() => setShowAgenda(false)} className="w-8 h-8 rounded-lg bg-white/10 text-white hover:bg-white/20 transition-colors">✕</button>
            </div>
            ...
            {clubAvailability && (
              <div className="mb-4 p-3 rounded-lg bg-white/5 border border-white/10">
                <div className="flex items-center gap-2 mb-1">
                  <div className={`w-3 h-3 rounded-full ${clubAvailability.available ? 'bg-green-500' : 'bg-red-500'}`}></div>
                  <span className="text-white font-medium">
                    {clubAvailability.available ? 'Ouvert maintenant' : 'Fermé'}
                  </span>
                </div>
                <p className="text-gray-300 text-sm">{clubAvailability.schedule}</p>
              </div>
            )}
            ...
            {profile.workingHours && (
              <div className="mb-4">
                ...
                <div className="p-3 rounded-lg bg-white/5 border border-white/10">
                  <p className="text-gray-300 text-sm">{profile.workingHours}</p>
                </div>
              </div>
            )}

            <div className="text-center text-gray-400 text-sm">
              💡 Contactez le club pour prendre rendez-vous
            </div>
          </div>
        </div>
      )}
